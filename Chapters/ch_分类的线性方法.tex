...
Logistic 回归直接对后验概率的 log-odds 建模为 $x$ 的线性函数，同时确保概率之和为 1 且落在 $[0,1]$ 内。对 $K$ 类问题：
...
\end{equation}
且 $\Pr(G = K \mid X = x) = 1 / \big(1 + \sum_{\ell=1}^{K-1} \exp(\beta_{\ell0} + \beta_\ell^\top x)\big)$。记全部参数集为 $\theta = \{\beta_{10}, \beta_1, \ldots, \beta_{(K-1)0}, \beta_{K-1}\}$，概率记为 $p_k(x; \theta)$。

当 $K=2$ 时退化为 §4.1 中的二分类形式，广泛用于生物统计中的二值响应问题（存活/死亡、患病/未患病等）。
